...
def main(searchWords,maxNumberAbstracts):
    ...
    createExcelFile(mostCommonWords)
...
def nltkAbstractTextMining(abstracts, record):
 #NLTK abstract search
    ...
    wordsNotNeeded = ["CONCLUSIONS", "CONCLUSION", "RESULTS", "BACKGROUND", "SIGNIFICANCE", "induced", "METHODS", "DNA"]
...
def createExcelFile(mostOccuringWords, dictionaryOfAllPubMedIds):
    try:
        ...
        Pub = wb.create_sheet(title="PubMed")
        ...
        ws.title = "Woorden"
        count =1
        for key, value in mostOccuringWords.items() :
            KeyCol = "A"+str(count)
            ValCol = "B"+str(count)
